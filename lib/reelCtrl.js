...
function ReelCtrl(options) {
  ...
  self.notifications = new Barnacles( { delayMilliseconds: 480,
                                        minDelayMilliseconds: 120,
                                        historyMilliseconds: 8000,
                                        disappearanceMilliseconds: 16000,
                                        keepAliveMilliseconds: 8000 });
...
function handleVisibilityEvent(instance, tiraid) {
  ...
  for(var cOffset = 0; cOffset < instance.reelMap.length; cOffset++) {
    ...
    for(var cDecoding = 0; cDecoding < decodings.length; cDecoding++) {
      if(decodings[cDecoding] && 
         decodings[cDecoding].identifier.hasOwnProperty('value')) {
  ...
  event.event = 'decoding';
  event.rssiMap = rssiMap;
  event.rssi = decodings[0].rssi;
  event.tiraid = tiraid;
 ...
 * @param {ReelCtrl} instance The ReelCtrl instance.
 * @param {Object} event The appearance event.
 */
function handleNotification(instance, event) {

  // Handle appearances only once a reel map is established
  if(instance.reelMap.length === 0) {
    return;
  }
